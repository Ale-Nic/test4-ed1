...
50. **¿Qué ocurre si delete[] se usa sobre un puntero no creado con new[]?**
A) Comportamiento indefinido
B) El programa sigue correctamente
C) Se libera solo el primer elemento
D) Se lanza una excepción

51. **¿Qué tipo de acceso permite leer un bloque completo de un archivo en una única operación?**
A) Secuencial
B) Directo con read()
C) getline()
D) >> con ifstream

52. **¿Qué salida produce?**
```cpp
...
108. **¿Qué valor imprime este fragmento?**
```cpp
int* p = new int(42);
delete p;
std::cout << *p;
